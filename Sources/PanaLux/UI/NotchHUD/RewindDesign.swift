...
// The standalone Rewind readout design from 40c96c4, before the engine existed. Kept next to the
// engine-driven RewindView so the two can be rendered side by side and one of them deleted.
// Nothing in the app uses these types; only RenderStillsTests draws them.

/// One thing that happened on the trail.
public struct DesignTrailMark: Equatable, Hashable, Identifiable {
...
public struct DesignRewindState: Equatable {
    ...
    public init(ago: TimeInterval, span: TimeInterval, marks: [DesignTrailMark],
...
public struct DesignRewindView: View {
    public let state: DesignRewindState
    private let accent = Color(red: 0.42, green: 0.78, blue: 1.0)

    public init(state: DesignRewindState) {
        self.state = state
    }
    ...
    public var body: some View {
        VStack(alignment: .leading, spacing: 7) {
            header
            DesignTrailTrack(state: state, accent: accent)
                .frame(height: 30)
...
struct DesignTrailTrack: View {
    let state: DesignRewindState
    let accent: Color
    ...
    @ViewBuilder
    private func markView(_ mark: DesignTrailMark, at x: CGFloat, mid: CGFloat) -> some View {
        switch mark.kind {
        case .edit:
            Capsule()
