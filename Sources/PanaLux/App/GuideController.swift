...
public struct TourStep {
    ...
    static let all: [TourStep] = [
        ...
        TourStep(anchor: .inspector, title: "Every key has two jobs",
                 body: "Turn on Program Buttons in the toolbar. Press any physical button to edit On tap, or hold it to select While held. Drop an action onto that button. USER, LOOP, both Shift keys and transport buttons work the same way. Turn Program Buttons off to use your map.",
                 goal: .none, highlight: nil),
        TourStep(anchor: .map, title: "Travel through your edit",
                 body: "With the default map, hold Undo and turn the centre ring. Every small movement scrubs the supported sliders; a slow full click reaches the next recorded stop. Turn faster to travel farther. Reverse the ring to move forward. Release Undo to keep what you see.",
                 goal: .none, highlight: "UNDO"),
        TourStep(anchor: .toolbar, title: "Keep another direction",
                 body: "Editing after rewinding starts a tangent and keeps your earlier work. Open Tangents to name versions or view another one. Choose a source and check only the sliders you want, then Merge into New Tangent. Both sources remain saved; masks and crop are not copied by the slider merge.",
                 goal: .none, highlight: nil),
